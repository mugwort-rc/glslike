...
PYBIND11_MODULE(__glslike, module) {
    module
        ...
        .def("length", static_cast<float(*)(const vec2 &)>(&length<float>))
        .def("normalize", static_cast<vec2(*)(const vec2 &)>(&normalize<float>))
        .def("exp", static_cast<vec2(*)(const vec2 &)>(&exp<float>))
        .def("pow", static_cast<vec2(*)(const vec2 &, float)>(&pow<float>))
        .def("dot", static_cast<float(*)(const vec2 &, const vec2 &)>(&dot<float>))
        // vec3
        .def("clamp", static_cast<vec3(*)(const vec3 &, float, float)>(&clamp<float>))
        .def("length", static_cast<float(*)(const vec3 &)>(&length<float>))
        .def("normalize", static_cast<vec3(*)(const vec3 &)>(&normalize<float>))
        .def("exp", static_cast<vec3(*)(const vec3 &)>(&exp<float>))
        .def("pow", static_cast<vec3(*)(const vec3 &, float)>(&pow<float>))
        .def("dot", static_cast<float(*)(const vec3 &, const vec3 &)>(&dot<float>))
        // vec4
        .def("clamp", static_cast<vec4(*)(const vec4 &, float, float)>(&clamp<float>))
        .def("length", static_cast<float(*)(const vec4 &)>(&length<float>))
        .def("normalize", static_cast<vec4(*)(const vec4 &)>(&normalize<float>))
        .def("exp", static_cast<vec4(*)(const vec4 &)>(&exp<float>))
        .def("pow", static_cast<vec4(*)(const vec4 &, float)>(&pow<float>))
        .def("dot", static_cast<float(*)(const vec4 &, const vec4 &)>(&dot<float>))
        ;
